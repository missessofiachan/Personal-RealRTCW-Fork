...
* **Upward Projection & Ceiling Shadow Guard (`tr_shadows.c`)**: Implemented ground distance clamping (`h = max(0, h)`) and shadow plane origin validation (`shadowPlaneZ <= origin[2] + 4.0f`) in `RB_ProjectionShadowDeform`. This guarantees that planar shadows project downward onto ground floors and ledges underfoot, eliminating floating shadows on upper wooden beams, ceilings, or mid-air geometry.
* **Automatic Crash Callstack Logger & Diagnostic Telemetry (`sys_main.c`)**: Installed automatic signal trap handling (`SIGSEGV`, `SIGFPE`, `SIGILL`, `SIGABRT`, `SIGBUS`) and `Sys_Error` diagnostics. Translates raw signal codes into descriptive error names, captures a 32-frame callstack backtrace via `backtrace()`, appends formatted entries to a persistent `crash.log` file with date/timestamps, and triggers a graphical `Sys_ErrorDialog` popup for immediate user feedback.
